	.file	"maximum_table.cpp"
	.intel_syntax noprefix
	.text
	.local	_ZStL8__ioinit
	.comm	_ZStL8__ioinit,1,1
	.globl	main
	.type	main, @function
main:
.LFB1731:
	.cfi_startproc
	endbr64
	push	rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	mov	rbp, rsp
	.cfi_def_cfa_register 6
	sub	rsp, 944
	mov	rax, QWORD PTR fs:40
	mov	QWORD PTR -8[rbp], rax
	xor	eax, eax
	lea	rax, -932[rbp]
	mov	rsi, rax
	lea	rax, _ZSt3cin[rip]
	mov	rdi, rax
	call	_ZNSirsERi@PLT
	mov	DWORD PTR -928[rbp], 1
	jmp	.L2
.L5:
	mov	DWORD PTR -924[rbp], 1
	jmp	.L3
.L4:
	mov	eax, DWORD PTR -924[rbp]
	movsx	rcx, eax
	mov	eax, DWORD PTR -928[rbp]
	movsx	rdx, eax
	mov	rax, rdx
	sal	rax, 4
	sub	rax, rdx
	add	rax, rcx
	mov	DWORD PTR -912[rbp+rax*4], 1
	add	DWORD PTR -924[rbp], 1
.L3:
	mov	eax, DWORD PTR -932[rbp]
	cmp	DWORD PTR -924[rbp], eax
	jle	.L4
	add	DWORD PTR -928[rbp], 1
.L2:
	cmp	DWORD PTR -928[rbp], 1
	jle	.L5
	mov	DWORD PTR -920[rbp], 2
	jmp	.L6
.L9:
	mov	eax, DWORD PTR -920[rbp]
	movsx	rdx, eax
	mov	rax, rdx
	sal	rax, 4
	sub	rax, rdx
	sal	rax, 2
	add	rax, rbp
	sub	rax, 908
	mov	DWORD PTR [rax], 1
	mov	DWORD PTR -916[rbp], 2
	jmp	.L7
.L8:
	mov	eax, DWORD PTR -920[rbp]
	lea	edx, -1[rax]
	mov	eax, DWORD PTR -916[rbp]
	movsx	rcx, eax
	movsx	rdx, edx
	mov	rax, rdx
	sal	rax, 4
	sub	rax, rdx
	add	rax, rcx
	mov	ecx, DWORD PTR -912[rbp+rax*4]
	mov	eax, DWORD PTR -916[rbp]
	sub	eax, 1
	movsx	rsi, eax
	mov	eax, DWORD PTR -920[rbp]
	movsx	rdx, eax
	mov	rax, rdx
	sal	rax, 4
	sub	rax, rdx
	add	rax, rsi
	mov	eax, DWORD PTR -912[rbp+rax*4]
	add	ecx, eax
	mov	eax, DWORD PTR -916[rbp]
	movsx	rsi, eax
	mov	eax, DWORD PTR -920[rbp]
	movsx	rdx, eax
	mov	rax, rdx
	sal	rax, 4
	sub	rax, rdx
	add	rax, rsi
	mov	DWORD PTR -912[rbp+rax*4], ecx
	add	DWORD PTR -916[rbp], 1
.L7:
	mov	eax, DWORD PTR -932[rbp]
	cmp	DWORD PTR -916[rbp], eax
	jle	.L8
	add	DWORD PTR -920[rbp], 1
.L6:
	mov	eax, DWORD PTR -932[rbp]
	cmp	DWORD PTR -920[rbp], eax
	jle	.L9
	mov	eax, DWORD PTR -932[rbp]
	mov	edx, DWORD PTR -932[rbp]
	movsx	rcx, edx
	movsx	rdx, eax
	mov	rax, rdx
	sal	rax, 4
	sub	rax, rdx
	add	rax, rcx
	mov	eax, DWORD PTR -912[rbp+rax*4]
	mov	esi, eax
	lea	rax, _ZSt4cout[rip]
	mov	rdi, rax
	call	_ZNSolsEi@PLT
	mov	rdx, QWORD PTR _ZSt4endlIcSt11char_traitsIcEERSt13basic_ostreamIT_T0_ES6_@GOTPCREL[rip]
	mov	rsi, rdx
	mov	rdi, rax
	call	_ZNSolsEPFRSoS_E@PLT
	mov	eax, 0
	mov	rdx, QWORD PTR -8[rbp]
	sub	rdx, QWORD PTR fs:40
	je	.L11
	call	__stack_chk_fail@PLT
.L11:
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE1731:
	.size	main, .-main
	.type	_Z41__static_initialization_and_destruction_0ii, @function
_Z41__static_initialization_and_destruction_0ii:
.LFB2231:
	.cfi_startproc
	endbr64
	push	rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	mov	rbp, rsp
	.cfi_def_cfa_register 6
	sub	rsp, 16
	mov	DWORD PTR -4[rbp], edi
	mov	DWORD PTR -8[rbp], esi
	cmp	DWORD PTR -4[rbp], 1
	jne	.L14
	cmp	DWORD PTR -8[rbp], 65535
	jne	.L14
	lea	rax, _ZStL8__ioinit[rip]
	mov	rdi, rax
	call	_ZNSt8ios_base4InitC1Ev@PLT
	lea	rax, __dso_handle[rip]
	mov	rdx, rax
	lea	rax, _ZStL8__ioinit[rip]
	mov	rsi, rax
	mov	rax, QWORD PTR _ZNSt8ios_base4InitD1Ev@GOTPCREL[rip]
	mov	rdi, rax
	call	__cxa_atexit@PLT
.L14:
	nop
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE2231:
	.size	_Z41__static_initialization_and_destruction_0ii, .-_Z41__static_initialization_and_destruction_0ii
	.type	_GLOBAL__sub_I_main, @function
_GLOBAL__sub_I_main:
.LFB2232:
	.cfi_startproc
	endbr64
	push	rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	mov	rbp, rsp
	.cfi_def_cfa_register 6
	mov	esi, 65535
	mov	edi, 1
	call	_Z41__static_initialization_and_destruction_0ii
	pop	rbp
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE2232:
	.size	_GLOBAL__sub_I_main, .-_GLOBAL__sub_I_main
	.section	.init_array,"aw"
	.align 8
	.quad	_GLOBAL__sub_I_main
	.hidden	__dso_handle
	.ident	"GCC: (Ubuntu 11.4.0-1ubuntu1~22.04.2) 11.4.0"
	.section	.note.GNU-stack,"",@progbits
	.section	.note.gnu.property,"a"
	.align 8
	.long	1f - 0f
	.long	4f - 1f
	.long	5
0:
	.string	"GNU"
1:
	.align 8
	.long	0xc0000002
	.long	3f - 2f
2:
	.long	0x3
3:
	.align 8
4:
