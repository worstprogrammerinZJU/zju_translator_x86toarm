...
"""
ARM64_O0 后缀文件清理脚本
功能：安全删除带有 _arm64_O0.s 后缀的文件和文件夹
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'arm64_cleanup_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class Arm64O0FileCleaner:
    """ARM64_O0 文件清理器"""
    
    # 定义目标后缀模式
    TARGET_SUFFIXES = {
        'primary': [
            '_arm64_O0.s', '_arm64_O0.S',  # 主要目标后缀
            '_aarch64_O0.s', '_aarch64_O0.S'  # ARM64的别名
        ],
        'variations': [  # 可能的变体
            '_arm64_o0.s', '_arm64_o0.S',
            '_aarch64_o0.s', '_aarch64_o0.S',
            '_arm64_O0', '_arm64_o0',
            '_aarch64_O0', '_aarch64_o0',
            'arm64_O0.s', 'arm64_o0.s',
            'aarch64_O0.s', 'aarch64_o0.s'
        ],
        'related_optimizations': [  # 相关优化级别
            '_arm64_O1.s', '_arm64_O2.s', '_arm64_O3.s',
            '_arm64_Os.s', '_arm64_Oz.s',
            '_aarch64_O1.s', '_aarch64_O2.s', '_aarch64_O3.s',
            '_aarch64_Os.s', '_aarch64_Oz.s'
        ],
        'other_arm_formats': [  # 其他ARM相关格式
            '_arm_O0.s', '_arm_o0.s', '_arm_O1.s',
            '_armv7_O0.s', '_armv8_O0.s'
        ]
    }
    
    def __init__(self, dry_run: bool = False, backup: bool = False, 
                 include_variations: bool = False, include_related: bool = False,
                 include_other_arm: bool = False, min_size: int = 0, max_size: int = 0):
        self.dry_run = dry_run
        self.backup = backup
        self.include_variations = include_variations
        self.include_related = include_related
        self.include_other_arm = include_other_arm
        ...
        if self.backup:
            ...
            self.backup_dir = Path(f"backup_arm64_files_{timestamp}")
    ...
    def get_target_patterns(self) -> List[str]:
        ...
        if self.include_related:
            patterns.extend(self.TARGET_SUFFIXES['related_optimizations'])
        
        if self.include_other_arm:
            patterns.extend(self.TARGET_SUFFIXES['other_arm_formats'])
    ...
    def is_target_file(self, filename: str) -> Tuple[bool, str, str]:
        ...
        for pattern in patterns:
            ...
            if filename.endswith(pattern):
                return True, pattern, f"精确匹配后缀: {pattern}"
            
            # 正则表达式匹配（处理大小写变体）
            regex_pattern = pattern.replace('_', '[_-]?').replace('O0', '[Oo]0')
            if re.search(f'{regex_pattern}$', filename, re.IGNORECASE):
                return True, pattern, f"正则匹配: {pattern}"
        
        # 检查文件名中包含目标关键词
        arm_keywords = ['arm64_O0', 'arm64_o0', 'aarch64_O0', 'aarch64_o0', 
                       'arm_O0', 'armv7_O0', 'armv8_O0']
        for keyword in arm_keywords:
            if keyword.lower() in filename.lower():
                return True, 'keyword', f"包含ARM关键词: {keyword}"
    ...
    def is_target_folder(self, foldername: str) -> Tuple[bool, str, str]:
        """判断文件夹是否为ARM相关文件夹"""
        folder_patterns = [
            'arm64', 'aarch64', 'arm', 'arm_assembly', 'arm64_assembly',
            'arm_O0', 'arm64_O0', 'aarch64_O0', 'matched_pairs_arm',
            'test_arm', 'test_arm64', 'test_aarch64'
        ]
        
        for pattern in folder_patterns:
            if pattern.lower() in foldername.lower():
                return True, pattern, f"文件夹匹配: {pattern}"
    ...
    def should_process_folder(self, folderpath: Path) -> Tuple[bool, str]:
        ...
        try:
            # 检查文件夹是否包含重要文件
            important_files = ['.git', 'README.md', 'Makefile', 'CMakeLists.txt']
            for file in folderpath.iterdir():
                if file.name in important_files:
                    return False, f"包含重要文件: {file.name}"
    ...
    def delete_file(self, filepath: Path) -> bool:
        ...
        try:
            ...
            # 记录匹配模式
            pattern = self._get_pattern_from_filename(filepath.name)
            if pattern:
                self.stats['patterns_matched'][pattern] = self.stats['patterns_matched'].get(pattern, 0) + 1
    ...
    def _get_pattern_from_filename(self, filename: str) -> str:
        """从文件名提取匹配模式"""
        for pattern in self.get_target_patterns():
            if filename.endswith(pattern):
                return pattern
    ...
    def delete_folder(self, folderpath: Path) -> bool:
        ...
        try:
            ...
            # 检查文件夹内容
            file_count = sum(1 for _ in folderpath.rglob('*') if _.is_file())
            logger.info(f"📁 删除文件夹: {folderpath.name} (包含 {file_count} 个文件)")
            ...
            for item in iterator:
                ...
                if item.is_file():
                    ...
                    if is_target:
                        ...
                        if self.stats['files_found'] <= 5:
                            status = "可处理" if should_process else "跳过"
                            logger.info(f"🔍 发现ARM64文件: {item.name} ({status})")
                            logger.info(f"   模式: {pattern} - {reason}")
                    ...
                    if is_target:
                        ...
                        if self.stats['folders_found'] <= 3:
                            status = "可处理" if should_process else "跳过"
                            logger.info(f"📁 发现ARM文件夹: {item.name} ({status})")
                            logger.info(f"   包含文件: {folder_info['file_count']} 个")
    ...
    def generate_report(self, target_files: List[Dict], target_folders: List[Dict], directory: Path) -> Dict:
        ...
        return {
            ...
            'config': {
                ...
                'include_other_arm': self.include_other_arm,
            ...
            'pattern_statistics': {
                ...
                'deletion_patterns': self.stats['patterns_matched']
            },
    ...
    def print_report(self, report: Dict):
        ...
        print(f"⚙️ 包含其他ARM格式: {'是' if config['include_other_arm'] else '否'}")
        print(f"🎯 目标模式数量: {len(patterns)} 种")
        print()
        ...
        print(f"  🗑️ 实际删除文件: {stats['files_deleted']}")
        print(f"  🗑️ 实际删除文件夹: {stats['folders_deleted']}")
        ...
        if pattern_stats['files']:
            print("🔍 文件匹配模式统计:")
            for pattern, count in sorted(pattern_stats['files'].items(), key=lambda x: x[1], reverse=True)[:8]:
                print(f"  {pattern}: {count} 个文件")
        
        if pattern_stats['deletion_patterns']:
            print("🗑️ 删除文件模式统计:")
            for pattern, count in sorted(pattern_stats['deletion_patterns'].items(), key=lambda x: x[1], reverse=True)[:5]:
                print(f"  {pattern}: {count} 个文件")
        print()
        ...
        if processed_files:
            print(f"🗑️ 处理的ARM64文件 ({len(processed_files)}):")
            for i, file in enumerate(processed_files[:10]):
                status = "[模拟] " if config['dry_run'] else ""
                size_info = self.format_size(file['size'])
                print(f"  {i+1:2d}. {file['name']} ({size_info})")
                print(f"       模式: {file['pattern']}")
                print(f"       修改: {file['modified_time']}")
        ...
        if processed_folders:
            print(f"📁 处理的ARM文件夹 ({len(processed_folders)}):")
            for i, folder in enumerate(processed_folders[:5]):
                status = "[模拟] " if config['dry_run'] else ""
                file_count = folder.get('file_count', 0)
                print(f"  {i+1}. {folder['name']} ({file_count} 个文件)")
                print(f"       模式: {folder['pattern']}")
        ...
        if config['dry_run']:
            ...
            if stats['files_deleted'] > 0 or stats['folders_deleted'] > 0:
                print(f"🎉 ARM64文件清理完成! 释放了 {stats['total_space_freed_formatted']} 空间")
                if stats['files_backed_up'] > 0:
                    print(f"💾 备份文件已保存到: {config['backup_dir']}")
            else:
                print("ℹ️ 没有找到可删除的ARM64目标文件")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="🛡️ ARM64_O0 文件清理工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  # 干运行模式预览
  python3 arm64_cleaner.py
  
  # 实际删除主要目标文件
  python3 arm64_cleaner.py --execute
  
  # 包含变体和相关文件
  python3 arm64_cleaner.py --execute --include-variations --include-related
  
  # 备份模式 + 指定目录
  python3 arm64_cleaner.py /path/to/project --execute --backup
  
  # 全面清理所有ARM相关文件
  python3 arm64_cleaner.py --execute --include-variations --include-related --include-other-arm
        """
    )
    ...
    parser.add_argument("--include-variations", "-v", action="store_true",
                       help="包含变体后缀（如 _arm64_o0.s, _aarch64_O0.s 等）")
    parser.add_argument("--include-related", "-r", action="store_true",
                       help="包含相关优化级别文件（如 _arm64_O1.s, _arm64_O2.s 等）")
    parser.add_argument("--include-other-arm", "-a", action="store_true",
                       help="包含其他ARM格式文件（如 _arm_O0.s, _armv7_O0.s 等）")
    ...
    parser.add_argument("--json", action="store_true",
                       help="输出JSON格式报告")
    parser.add_argument("--output", default="arm64_cleanup_report.json",
                       help="报告输出文件")
    ...
    try:
        ...
        # 创建清理器
        cleaner = Arm64O0FileCleaner(
            dry_run=not args.execute,
            backup=args.backup,
            include_variations=args.include_variations,
            include_related=args.include_related,
            include_other_arm=args.include_other_arm,
            min_size=args.min_size,
            max_size=args.max_size
        )
        
        logger.info(f"🔍 开始扫描ARM64文件目录: {target_dir}")
